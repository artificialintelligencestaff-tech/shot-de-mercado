...
Proposito:
    Obtener datos de derivados de Coinfuty MCP.
    Futures: precio, OI, funding, long/short, liquidaciones.
    Sin API key.
"""
...
def main():
    ...
    print(f"[YIN] Coinfuty MCP - {ts}")

    # 1. Market overview
    print("[YIN] Solicitando market overview...")
    overview = call_coinfuty("get_market_overview", {})
    overview_path = MERCADOS_DIR / f"market_overview_{ts}.json"
    with open(overview_path, "w", encoding="utf-8") as f:
        json.dump(overview, f, indent=2, ensure_ascii=False)
    print(f"[YIN] Guardado: {overview_path}")

    # 2. Funding rates de BTC
    print("[YIN] Solicitando funding rates de BTC...")
    funding = call_coinfuty("get_funding_rates", {"coin": "BTC"})
    ...
    # 3. Coin summary de ETH
    print("[YIN] Solicitando coin summary de ETH...")
    eth_summary = call_coinfuty("get_coin_summary", {"coin": "ETH"})
    ...
    print("[YIN] Script 15 finalizado.")
